...
public class Vault
{
	...
	private static string CARBON_ID => CARBON_ID_CACHE ??=
		JObject.Parse(OsEx.File.ReadText(Path.Combine(Defines.GetRustIdentityFolder(), "carbon.id")))["UID"].ToObject<string>();
